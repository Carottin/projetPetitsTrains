...
def playAllSound(test):
    ...
    global isPlaying
    print(index_soundToPlay.get())
    print("le son joue est : " )
    print(indexSoundPlayed)
    global canPlayMusic1
...
def get_parcour(parcour_path):
    ...
    for directory in os.listdir(parcour_path):
    ...
    for widget in frame_4.winfo_children():
        if(widget['text'] != "back"):
            widget.destroy()
    Label(frame_4 , text= "WAGON1",width=10,font=(None, 15)).pack()
    for language in language_list :
